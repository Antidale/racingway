...
### !cr
Rolls a seed for the the current community race flagset. use [!check_cr](#check_cr) to for details about the flagset.
...
### !check_cr
A command for restreamers to use to check what the currently configured community race settings are before rolling the seed. Always provides the flagset name and what site it's rolled from (main, alpha, galeswift). If size permits, will send the full flagstring.
